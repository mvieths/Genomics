...
public class BaumWelch
{
    ...
    /**
     * Uses the currentProbability, currentTransitionMatrix, and currentEmissionMatrix to generate state maps for
     *  the test data
     */
                ...
                else if (columns[0].equals("<>"))
                {
                    if (sequenceData.length() > 0)
                    {
                        ...
                        dataSets.add(new HMMDataSet(sequenceData.toString(), stateData.toString(), observations,
                                states, initialProbability, transitionMatrix, emissionMatrix));
